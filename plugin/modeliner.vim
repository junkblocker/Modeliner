...
"
" Last Change: Feb 25 2026 06:11 PM PST-Jun-2008.
...
if !exists('g:Modeliner_format')
    let g:Modeliner_format = 'et ff= fenc= fdl= fdm= sts= sw= ts='
    " /[ ,:]/ delimited.
    "
...
function! s:Modeliner_execute()
    ...
    let l:modeline = substitute(l:modeline, '^\s\+', '', '')
    let l:modeline = substitute(l:modeline, '\s\+$', '', '')
...
function! s:Commentify(s)
    let l:opts = substitute(a:s, '^\s\+', '', '')
    let l:commentstring = &commentstring
    if len(l:commentstring) == 0
        let l:commentstring = '%s'
    endif
    if exists('g:NERDMapleader') " NERDCommenter
        let l:result = b:left . ' vim: set ' . l:opts . ' : ' . b:right
    else
        " Normalise the commentstring so %s is preceded by exactly one space,
        " then substitute without a leading space in the replacement so the
        " result is always: <comment-leader> vim: set <opts> :
        let l:cs = substitute(l:commentstring, '\s*%s', ' %s', '')
        let l:result = substitute(l:cs, '%s', 'vim: set ' . l:opts . ' :', '')
    endif
